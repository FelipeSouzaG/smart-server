...
router.post('/', protect, async (req, res) => {
    const { items, total, customerName, customerWhatsapp, customerCnpjCpf, paymentMethod, discountApplied } = req.body;
    ...
    try {
        ...
        for (const saleItem of items) {
            ...
            if (saleItem.type === 'product') {
                // Fetch fresh product data to get current cost
                const product = await Product.findById(saleItem.item.id);
                if (product) {
                    itemCost = product.cost; // Use current average cost
                    // Deduct stock
                ...
                // For services, use the cost stored in the service definition
                const service = await Service.findById(saleItem.item.id);
                if (service) {
                    itemCost = service.partCost + service.serviceCost + service.shippingCost;
                }
            }

            saleTotalCost += itemCost * saleItem.quantity;
            
            // Add unitCost to the item stored in TicketSale for history
            updatedItems.push({
                ...saleItem,
                unitCost: itemCost
            });
